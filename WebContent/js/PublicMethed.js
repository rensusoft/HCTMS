	            ...
	            if(res.success==true){
	            	if(type==1){
	            		...
	            		layer.alert(content, {
	            			...
	            			offset : [(pHeight-180)/2+"px",(pWidth-300)/2+"px"],
	            		    closeBtn: 1
	            		});
	            	}else{
	            		successFun(res);
...
function clickToUrlTab(id,title,url){
	addTabs({
        id: id,
        title: title,
        content: "",
 ...
 * @return {}
 */
function HTMLChangeTxt(){
	//把所有checkbox勾选的变成钩
	$('input[type=checkbox]').each(function(){
		if($(this).is(':checked')){
			$(this).parent().html("<strong class=\"yes\">√</strong>");
		}else{
			$(this).parent().html("<strong class=\"non\"></strong>");
		}
	});
	//把所有单行输入框变成普通文字
 ...
 * 
 */
function verifyDigital(e){
	 e = window.event || e;  
     var k = e.keyCode || e.which;
     if(!(k==46)&&!(k==8)&&!(k==37)&&!(k==39)){
    	  if((k>=48&&k<=57)||(k>=96&&k<=105)){
    			  keychar = String.fromCharCode(k);
        		  numcheck = /^[0-9]\d*$/;
