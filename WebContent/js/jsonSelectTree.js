function jsonSelectTree(obj){
    ...
    function onBodyDown(event) {
		if (!(event.target.id == obj.selBtn || event.target.id == obj.treeContent || $(event.target).parents("#" + obj.treeContent).length>0)) {
			hideMenu();
			$("#" + obj.listShow).collapse('show');
		}
	}
