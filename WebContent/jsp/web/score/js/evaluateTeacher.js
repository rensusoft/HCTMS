...
//    			str += '' +
//    			'<span>' +
//    			'<a href="javascript:void(0);" onclick="formManage(' + res.data[i].form_id + ',-10' + ',-100,' + res.data[i].pub_num + ')">';
...
//    			str += '</a>' +
//    			'<i>' + res.data[i].name + '</i>' +
//    			'<input type="hidden" id="hidden_name_' + res.data[i].form_id + '" value="' + res.data[i].name + '"/>' +
////    			'<input type="hidden" id="hidden_flag_' + res.data[i].form_id + '" value="' + res.data[i].flag + '"/>' +
//    			'<input type="hidden" class="hidden_form_id" value="' + res.data[i].form_id + '"/>' +
//    			'</span>';
//    		}
////    	  	alert(str);
//    		$("#formName").html(str);
...
//    				'<td>' +
//    					'<div>' +
//    						'<b class="gray"  id="td' + i + '">' + (i+1) + '</b>' +
//    						'<i  class="iWidth gray"  id="i' + i + '"></i>' +
//    					'</div>' +
//    					'<div class="divv">' +
//    						'<span class="proc_name">' + res.data[i].proc_name + '</span>' +
//    					'</div>' +
//    				'</td>';
//    		}
//    		str += '<td class="finish_td" ><div><b class="gray" id="td' + num + '">✔</b></div>' +
//    			'<div class="divv"><span>出科</span></div></td>';
////    		alert(str);
//    		$(".flow_chart").html(str);
//    		
//    		if($("#process_top table tr td").size() == 4){
//    	        $("#process_top table tr td i").width(300);
//    	    }else if($("#process_top table tr td").size() == 5){
//    	        $("#process_top table tr td i").width(220);
//    	    }else if($("#process_top table tr td").size() == 6){
//    	        $("#process_top table tr td i").width(100);
//    	    }else if($("#process_top table tr td").size() == 3){
//    	        $("#process_top table tr td i").width(460);
//    	    }
//    	}
//    });
//}
...
////表单配置填写前的展示
//function formManage(form_id,flag_read,create_auth_id,pub_num){ 
...
//		var s_user_auth_id = stuId;
//		var s_orga_id = train_dept_id;
//		var url = "";
...
//			mypopdiv(2,"表单信息",'1150px',(pHeight-100)+'px','','','N',url);
...
function goGrade(exam_result){
	...
	if(exam_result == 1){
		...
		theory_sco = $("#theory_sco").val();
		skill_sco = $("#skill_sco").val();
//		if($("#textId").val()==''){
//			myMsg("请输入审批意见!",'50%','50%');
//			return false;
//		}
		if($("#theory_sco").val()!==''){
			if(isNaN($("#theory_sco").val())){
				myMsg("分数只能输入数字!",'50%','50%');
				return false;
			}else{
				if($("#theory_sco").val()>100||$("#theory_sco").val()<0){
					myMsg("分数只能是百分制!",'50%','50%');
					return false;
				}
			}
		}else{
			myMsg("请输入理论考试分数!",'50%','50%');
			return false;
		}
		if($("#skill_sco").val()!==''){
			if(isNaN($("#skill_sco").val())){
				myMsg("分数只能输入数字!",'50%','50%');
				return false;
			}else{
				if($("#skill_sco").val()>100||$("#skill_sco").val()<0){
					myMsg("分数只能是百分制!",'50%','50%');
					return false;
				}
			}
		}else{
			myMsg("请输入技能考试分数!",'50%','50%');
			return false;
		}
...
//				EXAM_SCO:$("#countId").val(),
				theory_sco:theory_sco,
				skill_sco:skill_sco,
				s_orga_id:train_dept_id,
