...
$(function(){
     ...
     for(var i=0;i<data.length; i++){
    	 var theory_sco;
  	      var skill_sco;
   	 if(data[i].theory_sco!=null&&data[i].theory_sco!="")
  	  	{
   		 theory_sco=data[i].theory_sco;
   	   		}
   	   	else{    	   		
   	   		theory_sco='暂无';    	   		
   	   	};
   	   	if(data[i].skill_sco!=null&&data[i].skill_sco!="")
   	  	{
   		 skill_sco=data[i].skill_sco;
   	   		}
   	   	else{   	   		
   	   		skill_sco='暂无';    	   		
   	   	};
   	str+=  '<div class="content_border ">';
   	if(data[i].train_status==51||data[i].train_status==""||data[i].train_status==null){
   		str+='<div class="section_boder opacity"> <span class="rotate hidden">'+
   		'<i></i><b>未轮转</b></span><div class="section">';
   		numW++;
   	}else if(data[i].train_status==52){
   	    orgaNameStr = data[i].orga_name;
   		str+='<div class="section_boder yellow_border opacity"> <span class="rotate">'+
   		'<i class="yellow"></i><b>待入科</b></span><div class="section">';
   		numZ++;
   	}else if(data[i].train_status==58){
   		str+='<div class="section_boder"> <span class="rotate">'+
   		'<i class="gary"></i><b>已轮转</b></span><div class="section">';
   		numS++;
   	}else if(data[i].train_status==53){
   		orgaNameStr = data[i].orga_name;
   		str+='<div class="section_boder"> <span class="rotate">'+
   		'<i class="green"></i><b>轮转中</b></span><div class="section">';
   		numS++;
    }else{
   	    orgaNameStr = data[i].orga_name;
   		str+='<div class="section_boder red_border"><span class="rotate">'+
   		'<i class="red"></i><b>待出科</b></span><div class="section">';
   	}
   	str+= '<p>'+data[i].orga_name+'</p>'+
   	  '<i>带教老师：<span style="color:#337AB7">'+data[i].teach_name+'</span>&nbsp;&nbsp;&nbsp;&nbsp;科主任：<span style="color:#337AB7">'+data[i].director_name+'</span>&nbsp;&nbsp;&nbsp;&nbsp;教学秘书：<span style="color:#337AB7">'+data[i].secretary_name+'</span></i>'+
   	  '</div>'+
   	  '<div class="indicatorContainer">'+
   	  '<div id="persent'+i+'"></div>'+
   	  '</div>'+
   	  '<div class="charge">'+
   	'<i>理论成绩：<span>'+theory_sco+'</span>&nbsp;&nbsp;&nbsp;'+
	     '技能成绩：<span>'+skill_sco+'</span>&nbsp;&nbsp;&nbsp;'+
   	  '<input type="text" readonly value="'+data[i].train_start_str+'"/>&nbsp;&nbsp;&nbsp;'+
   	  '<input type="text" readonly value="'+data[i].train_end_str+'"/>'+
   	  '</i>'+
   	  '</div>'+
   	  '</div>'+
   	  '</div>'+' <script> '+
      '$("#persent'+i+'").radialIndicator({'+
      '  barColor: "#FAA07C",'+
      '   barWidth: 8,'+
     ...
     str+= '  </div>';
     str='<p><span>已轮转<i>&nbsp;'+numS+'&nbsp;</i>个科室</span><span>正在轮转<i>&nbsp;'+orgaNameStr+'&nbsp;</i></span> <span>未轮转<i>&nbsp;'+numW+'&nbsp;</i>个科室</span></p> <div class="main_content" >'+str;
    $("#mainDiv").html(str);
	  }		 
          }
          });
//	
//	var url=ctx+'/teachweb/selectTrainPlan.action';
//	 var postData={};
...
//       	if(data[i].train_status==51||data[i].train_status==""||data[i].train_status==null){
//       		str+='<div class="section_boder opacity"> <span class="rotate hidden">';
//       	}else if(data[i].train_status==57){
//       		str+='<div class="section_boder"> <span class="rotate hidden">';
//       	}else{
//       		str+='div class="section_boder red_border"><span class="rotate">';
//       	}
//       	str+= '<i></i><b>'+data[i].proc_name+'</b></span><div class="section">'+
//       	  '<p>'+data[i].orga_name+'</p>'+
//       	  '<i>带教老师：<span>'+data[i].teach_name+'</span>&nbsp;&nbsp;&nbsp;&nbsp;科主任:<span>'+data[i].director_name+'</span></i>'+
//       	  '</div>'+
//       	  '<div class="indicatorContainer">'+
//       	  '<div id="persent1"></div>'+
//       	  '</div>'+
//       	  '<div class="charge">'+
//       	  '<i>出科成绩：<span><a href="">暂无</a></span>&nbsp;&nbsp;&nbsp;'+
//       	  '<input type="text" readonly value="'+data[i].train_start_time+'"/>&nbsp;&nbsp;&nbsp;'+
//       	  '<input type="text" readonly value="'+data[i].train_end_time+'"/>'+
//       	  '</i>'+
//       	  '</div>'+
//       	  '</div>'+
//       	  '</div>';
