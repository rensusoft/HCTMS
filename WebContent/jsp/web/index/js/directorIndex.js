...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function shuJu(){
	mypopdiv(1,"带教老师",'200px','300px','','','N',teachConet);
};
...
function getTeaCount(){
	var url=ctx+"/teachweb/getTeaCount.action"; 
	doAjax(url,"",2,function(res){	
		$("#teaCount").text(res.data);
		if(res.data==0){
			$("#teaCount").attr("style","color:#505F91;");
		}
			teachConet="<div style='text-align:center;'>";	
		for ( var i = 0; i < res.teacherList.length; i++) {
			teachConet+="<span  style=' background-color: lightblue; display: block;float:left;'><strong>姓名</strong>："+res.teacherList[i].name+"</span><br/>";
		}
		teachConet+="</div>";
	});
}
...
//出科
function chuKe(){
	clickToUrlTab("出科审核","出科审核",ctx+"/jsp/web/score/graduateCheckDirector.jsp");
}
